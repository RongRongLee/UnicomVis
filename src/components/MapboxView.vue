...
var accent = d3.scaleOrdinal(d3.schemeSet2);//定义图的颜色
...
export default {
  ...
  mounted() {
    ...
    DataProvider.getCellRelations().then(response => {//取基站间关系数据
    ...
    DataProvider.getCellSemantic().then(response => {//取基站语义数据
 ...
 */
     ...
     */
    mapLoadGeojson(that) {
      //this.map.on 为特定类型的事件添加监听器。
      this.map.on("move", this.updateLines);//move为添加的监听器的事件类型，移动鼠标，调用updatelInes方法

      this.map.on("load", function() {
        DataProvider.getMapJson().then(response => {//获取地图数据
          that.data = response.data;
          that.addRegion2Map(that.data);//地图加载时，调用addRegion2Map方法
        });

        DataProvider.getCellInfo().then(response => {//获取基站数据
        ...
        that.map.on("click", function(e) {
          ...
          points.forEach(function(p) {
            ...
            if (
              ...
              p["properties"].color = "red";//被连接的基站的颜色
